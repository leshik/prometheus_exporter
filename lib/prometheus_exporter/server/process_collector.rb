...
module PrometheusExporter::Server
  ...
  class ProcessCollector < TypeCollector
    MAX_PROCESS_COUNTER_AGE = 28800
    MAX_PROCESS_GAUGE_AGE = 60
    ...
    def metrics
      ...
      process_metrics.map do |m|
        ...
        metric_key.merge!(m.fetch('custom_labels', {}))
    ...
    def collect(obj)
      ...
      process_metrics.delete_if do |current|
        (obj['pid'] == current['pid'] && obj['hostname'] == current['hostname']) ||
          (current['created_at'] + MAX_PROCESS_COUNTER_AGE < now && !(current.keys & PROCESS_COUNTERS.keys.map(&:to_s)).empty?) ||
          (current['created_at'] + MAX_PROCESS_GAUGE_AGE < now && !(current.keys & PROCESS_GAUGES.keys.map(&:to_s)).empty?)
      end
